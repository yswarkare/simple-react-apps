...
import { KeyIcon, UserIcon, EmailIcon, SearchIcon, EyeSlash, Eye } from './input-ui-icons';
  ...
  return (
    <div className="w-full gap-1 flex flex-col items-start">
      ...
      <label className={`w-full input input-bordered flex items-center gap-2 ${error && 'input-error'}`}>
        {position === 'left' && <div className="tooltip tooltip-left" data-tip={label || type}><InputIcon type={type} /></div>}
        <input
        ...
        {type === 'password' && (
          <div className='cursor-pointer tooltip tooltip-right' data-tip={inType === 'text' ? 'hide' : 'show'} onClick={(e) => changeIcon(e)}>
            {
              inType === 'text' && <EyeSlash />
              || inType === 'password' && <Eye />
            }
          </div>
        )}
        {position === 'right' && <div className="tooltip tooltip-left" data-tip={label || type}><InputIcon type={type} /></div>}
      </label>
